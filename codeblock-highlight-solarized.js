// AI 辅助编写，用于在使用 solarized 主题时，将笔记中的代码块的颜色主题也覆盖成 solarized 主题
// 不太稳定，会出现刚打开时不是 solarized 主题，几秒后变成 solarized 主题的情况。
function trilliumGe(e) {
    return /\bcode\s+\.hljs-/.test(e) && (e = e.replace(/\bcode\.hljs/g, `.hljs`),
    e = e.replace(/\bcode\s+\.hljs-/g, `.hljs .hljs-`)),
    e = e.replace(/^\.hljs\s*\{/m, `.hljs, .ck-content pre.hljs {`),
    e
}
...
function hookHighlightStyle(css) {
    let style = document.getElementById("trilium-solarized-highlight");

    if (!style) {
        style = document.createElement("style");
        style.id = "trilium-solarized-highlight";
        document.head.appendChild(style);
    }

    style.textContent = trilliumGe(css);
}


function waitForTriliumHighlightStyle(css) {
    const isHighlightStyle = node =>
        node instanceof HTMLStyleElement &&
        node.textContent.includes(".hljs, .ck-content pre.hljs {");

    // 如果 Trilium 已经加载完成
    for (const style of document.head.querySelectorAll("style")) {
        if (isHighlightStyle(style)) {
            hookHighlightStyle(css);
            return;
        }
    }
    ...
    const observer = new MutationObserver(mutations => {
        for (const mutation of mutations) {
            for (const node of mutation.addedNodes) {
                if (node.textContent.includes(".hljs, .ck-content pre.hljs {")) {
                    observer.disconnect();
                    hookHighlightStyle(css);
                    return;
                }
...
if (document.body.dataset.themeId.startsWith('solarized-light')) {
    waitForTriliumHighlightStyle(solarized_light)
}
else if (document.body.dataset.themeId.startsWith('solarized-dark')) {
    waitForTriliumHighlightStyle(solarized_dark)
}
